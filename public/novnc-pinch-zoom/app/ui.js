...
* ------v------*/
    ...
    addZoomHandlers() {
        document.getElementById("noVNC_zoom_in_button")
            .addEventListener('click', UI.zoomIn);
        document.getElementById("noVNC_zoom_out_button")
            .addEventListener('click', UI.zoomOut);
        document.getElementById("noVNC_zoom_reset_button")
            .addEventListener('click', UI.zoomReset);
    },
 ...
 * ------v------*/
    ...
    zoomIn() {
        ...
        const currentScale = UI.rfb._display.zoomScale;
        const newScale = Math.min(3.0, currentScale * 1.2);
        ...
        UI.rfb._display.setPinchZoom(newScale, centerX, centerY);
        UI.updateZoomLevel();
    },
    ...
    zoomReset() {
        ...
        UI.rfb._display.resetPinchZoom();
        UI.updateZoomLevel();
    },
    ...
    updateZoomLevel() {
        ...
        const scale = UI.rfb._display.zoomScale;
        const percentage = Math.round(scale * 100);
        document.getElementById('noVNC_zoom_level').textContent = percentage + '%';
    },
 ...
 */
    ...
    // iframe 통신을 위한 postMessage 리스너 추가
    setupIframeMessaging() {
        window.addEventListener('message', (event) => {
            // 보안을 위해 origin 체크 (필요시 특정 도메인으로 제한)
            // if (event.origin !== 'http://localhost:5270') return;
            
            if (event.data && event.data.type === 'noVNC_zoom') {
                switch (event.data.action) {
                    ...
                    case 'getZoomLevel':
                        // 현재 줌 레벨을 부모 창에 전송
                        const currentScale = UI.rfb ? UI.rfb._display.zoomScale : 1.0;
                        const percentage = Math.round(currentScale * 100);
                        window.parent.postMessage({
                            type: 'noVNC_zoom_response',
